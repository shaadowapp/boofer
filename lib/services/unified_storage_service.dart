...
class UnifiedStorageService {
  ...
  static const String userHandle =
      'user_handle'; // @username (alphanumeric + underscore)
  ...
  static const String userDisplayName =
      'user_display_name'; // Will migrate to userFullName
  ...
  // Network Usage Keys
  static const String networkUsageMessagesSent = 'network_usage_messages_sent';
  static const String networkUsageMessagesReceived =
      'network_usage_messages_received';
  static const String networkUsageMediaSent = 'network_usage_media_sent';
  static const String networkUsageMediaReceived =
      'network_usage_media_received';
  static const String networkUsageCallsSent = 'network_usage_calls_sent';
  static const String networkUsageCallsReceived =
      'network_usage_calls_received';

  /// Increment network usage stats
  static Future<void> incrementNetworkUsage(
    String type,
    int bytes, {
    bool isSent = true,
  }) async {
    ...
    switch (type) {
      case 'messages':
        key = isSent ? networkUsageMessagesSent : networkUsageMessagesReceived;
        break;
      case 'media':
        key = isSent ? networkUsageMediaSent : networkUsageMediaReceived;
        break;
      case 'calls':
        key = isSent ? networkUsageCallsSent : networkUsageCallsReceived;
        break;
      default:
        return;
    }
  ...
  /// Reset network usage stats
  static Future<void> resetNetworkUsage() async {
    final prefs = await _prefs;
    await prefs.setInt(networkUsageMessagesSent, 0);
    await prefs.setInt(networkUsageMessagesReceived, 0);
    await prefs.setInt(networkUsageMediaSent, 0);
    await prefs.setInt(networkUsageMediaReceived, 0);
    await prefs.setInt(networkUsageCallsSent, 0);
    await prefs.setInt(networkUsageCallsReceived, 0);
  }
